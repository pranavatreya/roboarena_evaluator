...
def run_evaluation(setting: EvalConfig, evaluator_email: str, institution: str) -> None:
    ...
    hardcoded_ip_to_policy = {
        "ws5.roboarena-api-domain-name.online": "paligemma_fast_droid",
        "ws6.roboarena-api-domain-name.online": "pi0_droid",
        "ws1.roboarena-api-domain-name.online": "pi0_fast_droid",
        "ws7.roboarena-api-domain-name.online": "paligemma_fast_specialist_droid",
        "ws4.roboarena-api-domain-name.online": "paligemma_diffusion_droid",
        "ws0.roboarena-api-domain-name.online": "paligemma_vq_droid",
        "ws2.roboarena-api-domain-name.online": "paligemma_binning_droid",
        "ws3.roboarena-api-domain-name.online": "pi05_droid",
        "roboarena-policy-server.chickenkiller.com": "dam",
    }
    hardcoded_policy_to_ip = {}
    unique_policy_names = []
    for k, v in hardcoded_ip_to_policy:
	hardcoded_policy_to_ip[v] = k
	unique_policy_names.append(v)
    
    letters = ["A", "B", "C", "D", "E", "F", "G", "H", "I"]
    print("These are the list of policies you can choose from to evaluate:")
    for i in range(9):
	letter = letters[i]
	policy_name = unique_policy_names[i]
	print(f"{letter}: {policy_name}")
    selection = input("Enter the letter (A through I) of the policy you want to evaluate: ")
    selection = selection.upper()
    assert selection in letters, "Invalid letter entered, exiting..."
    policy_selected = ""
    for i in range(9):
        if letters[i] == selection:
	    policy_selected = unique_policy_names[i]
	    break
    assert policy_selected != ""
    ...
    lang_command = input("Natural-language command to send to the policy: ")
